...
class Picture < ActiveRecord::Base
  ...
  def story(gender)
    ...
    story.title = %(<fb:name uid="#{self.fb_user_id}" useyou="false" capitalize="true" /> took a picture of #{gender == 'male' ? 'him' : 'her'}self today)
    story.image_1 = self.thumb.authenticated_s3_url
    story.image_1_link = "http://apps.facebook.com/apictureeveryday/pictures/#{self.id}"
    story.body = %(<a href="http://apps.facebook.com/apictureeveryday">Take a picture of yourself with your webcam today</a>.)
    story
  end
